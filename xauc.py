...
def get_dr_lr(x_train, y_train, A_train, Rhat_train, THRESH_1, THRESH_0, quiet = True):
    # Density ratio estimation (quick via log reg)
    a_1_conf = (A_train == 0)&(y_train==1)&(Rhat_train < THRESH_1)
    b_0_conf = (A_train == 1)&(y_train==0)&(Rhat_train > THRESH_0)
    # Let Ytilde = 1 be a_1_conf
    # subset data
    x_train_subset = np.vstack([x_train[ (A_train == 0)&(y_train==1)&(Rhat_train < 0.5)], x_train[(A_train == 1)&(y_train==0)&(Rhat_train > 0.5)]])
    Y_tilde = np.zeros(x_train_subset.shape[0])
    Y_tilde[0:sum((A_train == 0)&(y_train==1)&(Rhat_train < 0.5))] = 1
    # density ratio estimation
    [dr_lr, Rhat_p1] = get_lr(x_train_subset, Y_tilde)
    # density ratios
    rho = (Rhat_p1 / (1- Rhat_p1)) /(np.mean(Y_tilde) / np.mean(1-Y_tilde))
    if not quiet:
        print x_train_subset.shape
        print (np.mean(Y_tilde) / np.mean(1-Y_tilde))
        print 'accuracy',dr_lr.score(x_train_subset, Y_tilde)
    return [rho, dr_lr, Rhat_p1, x_train_subset, Y_tilde]

def get_subgroup_mean(x_train_subset,x_train,rho,dr_radius=0.9,diff_thresh=0.01, quiet = True):
#     plt.figure()
    avg_covs_subgroup = np.round(np.mean(x_train_subset[ np.where( np.abs(rho - 1) < dr_radius )[0] ,:], axis = 0  ),2)
    avg_covs_pop = np.round(np.mean(x_train, axis = 0  ),2)
    if not quiet:
        print len(x_train_subset[ np.where( np.abs(rho - 1) < dr_radius )[0] ,1])
#     plt.hist(x_train_subset[ np.where( np.abs(rho - 1) < dr_radius )[0] ,1],density=True, alpha = 0.5)
#     plt.hist(x_train['age'], density = True, alpha = 0.5) # sns.kdeplot
        print (avg_covs_subgroup - avg_covs_pop )[np.abs(avg_covs_pop - avg_covs_subgroup ) > diff_thresh ]
    return [avg_covs_subgroup, avg_covs_pop]
